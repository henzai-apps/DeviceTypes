...
class DeviceTypesDecoderTests: XCTestCase {
    let example = """
    == Device Types ==
    ...
    func testParseStdOut() {
        let decoder = DeviceTypesDecoder()
        let types = try! decoder.decode(stdout: example)
        XCTAssertEqual(types.count, 68)
        XCTAssertEqual(types[0].name, "iPhone 4s")
        XCTAssertEqual(types[0].identifier, "com.apple.CoreSimulator.SimDeviceType.iPhone-4s")
    }
}
